...
!-------------------------------------------------------------------------
 USE general_routines  ;  USE environmental_routines
 IMPLICIT NONE
 INTEGER,PARAMETER::iwp=SELECTED_REAL_KIND(15)
 INTEGER::airin,fixed_freedoms_1,fixed_freedoms_2,fluxin,hfbc,htbc,i,    &
   ibound,iel,igroup,indic,iout,iside,j,jdate,jhour,ndim,nels,neq,nfiles,&
   nip,nip_s,nn,nod,nod_s,npri,nprops,np_types,nsensors,nside,nstep,tempin
 REAL(iwp)::ab,azimuth,beta,cos_alpha,cos_z,cp,d1,delta,det,dtim,htco,   &
   htco1,htco2,ibeam,idiffuse,jd,jd_i,jd_f,n1,n2,n3,omega,               &
   penalty=1.0e20_iwp,phi,rho,t_a,t_a1,t_a2,theta,ucte,val0,val1,val2,yy
...
!-------------------dynamic arrays----------------------------------------
 ...
 REAL(iwp),ALLOCATABLE::bk(:),bkcopy(:),bp(:),bpcopy(:),coord(:,:),      &
   coord_s(:,:),der(:,:),der_s(:,:),deriv(:,:),elt(:),elt_s(:),fun(:),   &
   fun_s(:),g_coord(:,:),jac(:,:),jac_s(:,:),kay(:,:),kp(:,:),kp_s(:,:), &
   loads(:),loadsh(:),loadsq(:),newlo(:),ntn(:,:),ntn_s(:,:),oldlo(:),   &
   pm(:,:),points(:,:),points_s(:,:),prop(:,:),sensors(:,:),storbp_1(:), &
   storbp_2(:),value_1(:),value_2(:),value_3(:),weights(:),weights_s(:)
...
!
 ...
 nprops=ndim+4
 neq=nn
 ALLOCATE(coord(nod,ndim),der(ndim,nod),deriv(ndim,nod),elt(nod),        &
  etype(nels),fun(nod),g_coord(ndim,nn),g_num(nod,nels),jac(ndim,ndim),  &
  kay(ndim,ndim),kdiag(neq),kp(nod,nod),num(nod),ntn(nod,nod),           &
  pm(nod,nod),points(nip,ndim),weights(nip))
!
...
!------------------boundary radiation load vector assembly----------------
 ...
 IF(hfbc/=0)THEN
   ...
   CASE(1)
     ...
     END IF
   CASE(2)
     CALL Kumar_model(d1,cos_z,ibeam,idiffuse)
   CASE(3)
...
!------------------write sensors temperatures-----------------------------
   ...
   IF(nsensors/=0)THEN
     elements_6: DO  i=1,nsensors
       ...
       iout=1000+i 
       WRITE(iout,'(5a,3x,5a,E15.4)')char_num(1:4),'-',char_num(5:6),'-',&
